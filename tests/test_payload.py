...
class PayloadTest(unittest.TestCase):
    def test_quick_reply(self):
        ...
        self.assertEquals(
            '{"content_type": "text", "payload": "PICK_YES", "title": "Yes"}',
            utils.to_json(q),
        )

    def test_quick_reply_shortcut(self):
        q = Payload.Message.convert_shortcut_quick_reply(
            [{"title": "Yes", "payload": "PICK_YES"}]
        )
    ...
    def test_message(self):
        ...
        with self.assertRaises(ValueError):
            m = Payload.Message(
                text="hello",
                quick_replies=Payload.QuickReply(title="Yes", payload="PICK_YES"),
            )
        ...
        self.assertEquals(
            '{"attachment": null, "metadata": "METADATA", '
    ...
    def test_payload(self):
        ...
        with self.assertRaises(ValueError):
            Payload.Payload(
                recipient=recipient, message=message, sender_action="NEW_ACTION"
            )
        ...
        self.assertEquals(
            ...
            ' "sender_action": "typing_off", "tag": null}',
            utils.to_json(p),
        )
        ...
        self.assertEquals(
            ...
            '[{"content_type": "text", "payload": "PICK_YES", "title": "Yes"}], "text": "hello"},'
            ' "notification_type": "REGULAR", "recipient": {"id": 123456, "phone_number": "+8210"},'
            ' "sender_action": "typing_off", "tag": "PAIRING_UPDATE"}',
            utils.to_json(p),
        )
